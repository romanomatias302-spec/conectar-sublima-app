## global

- onboarding
- mejorar modo oscuro

- mejorar sistema de usuarios es decir los nombres de los privilegios, tenemos a un usuario con nombre de rol "usuario"

- historial de movimientos (para auditorias)

- notificaciones
- cambiar formato fecha segun pais
- cambiar idiaoma segun pais
- verificar accion volver (para volver correctamente a la pagina anterior visitada)

## clientes

## modulo configuracion

- al habilitar el swich de zona de impresion, si queremos que la imagen aparezca en el form a mostrar en pedidos, debemos entrar y guardar. mejorar eso. Deberia cargar todo a penas habilitamos

- Poner orden manual de los swich para que aparezcan en ese orden en el formulario

- futura integracion por render, opcion PRO para planes mas grande

## inicio

- poner graficos en las tarjetas de arriba.

## vista dueño saas

- armar un menu con los modulos correspondientes para la gestion de nuestros clientes

## futuros modulos

- gastos
- historial
- Tandas (hacer dinamicas las columnas de produccion para crear tandas)
- incidencias (ver como se puede resolver cuando tenemos faltantes)
- Toma de pedidoS, WEB PUBLICA, los clientes de cada cliente saas podra compartir un enlace para cargar pedido.
- Presupuesto (vincular con ventas)

## ventas

- ver de integrar factura electronica / presupuestos
- mejorar estetica

## informes

listo

## produccion

cuando se crea una columna nueva se resetean los tiempo, mejorar eso

- al soltar tarjeta mejorar experiencia
- columnas automatizadas, reglas entrada y salida, creacion de tandas etc.
